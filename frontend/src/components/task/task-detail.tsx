...
import type { ProjectMember, Task, TaskPriority, TaskStatus, UpdateTaskFormData } from '~/types'
...
const statusConfig: Record<TaskStatus, { label: string; active: string; idle: string }> = {
  pending: {
    label: 'To Do',
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      ...
      >
        ...
        <div className="flex flex-1 overflow-hidden">
          ...
          {/* Sidebar */}
          <div className="w-48 shrink-0 border-l overflow-y-auto px-3.5 py-4 space-y-5">
            {/* Status */}
            <div>
              ...
              <div className="flex flex-col gap-0.5">
                {(Object.keys(statusConfig) as TaskStatus[]).map((s) => (
                  <button
                    ...
                    onClick={() => {
                      if (canEdit && task.status !== s) onUpdate(task.id, { status: s })
                    }}
                    className={cn(
                      'w-full rounded-md px-2.5 py-1.5 text-left text-xs font-medium transition-colors disabled:cursor-default',
                      task.status === s ? statusConfig[s].active : statusConfig[s].idle,
                    )}
                  >
            ...
            <div>
              ...
              <Select
                value={task.priority}
                onValueChange={(val) => {
                  if (canEdit) onUpdate(task.id, { priority: val as TaskPriority })
                }}
                disabled={!canEdit || isUpdating}
              >
            ...
            <div>
              ...
              <input
                ...
                onChange={(e) => {
                  if (canEdit) onUpdate(task.id, { due_date: e.target.value || undefined })
                }}
            ...
            <div>
              ...
              <input
                ...
                onChange={(e) => {
                  if (canEdit) onUpdate(task.id, { start_date: e.target.value || undefined })
                }}
                className="w-full rounded-md border bg-background px-2 py-1.5 text-xs outline-none focus:ring-1 focus:ring-primary disabled:opacity-50 transition-shadow"
              />
            </div>
